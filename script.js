...
function openProjectDetail(key, addHistory = true) {
  if (addHistory) history.pushState({scempyy:true, screen:'project', key}, '', location.pathname + '#project-' + encodeURIComponent(key));
...
function openInfoPage(type, addHistory = true){
  if (addHistory) history.pushState({scempyy:true, screen:'info', type}, '', location.pathname + '#'+encodeURIComponent(type));
  const data={
...
function openProjects(addHistory = true){
  if(addHistory) history.pushState({scempyy:true, screen:'projects'}, '', location.pathname + '#projects');
  closeAllScreens();
  projectsModal.classList.add('open');
}
function openContacts(addHistory = true){
  if(addHistory) history.pushState({scempyy:true, screen:'contacts'}, '', location.pathname + '#contact');
  closeAllScreens();
  contactsPanel.classList.add('open');
}
function handleBackNavigation(){
  // Use the browser history so Android/iOS Back buttons and gestures work.
  if (history.state && history.state.scempyy) history.back();
  else closeAllScreens();
}

// Treat the portfolio's in-page screens like real navigation states.
history.replaceState({scempyy:true, screen:'home'}, '', location.pathname);
